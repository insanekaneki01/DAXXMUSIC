...
from INSANEMUSIC.core.call import DAXX
from INSANEMUSIC.misc import sudo
...
async def init():
    ...
    LOGGER("INSANEMUSIC.plugins").info("𝐀𝐥𝐥 𝐅𝐞𝐚𝐭𝐮𝐫𝐞𝐬 𝐋𝐨𝐚𝐝𝐞𝐝 𝐁𝐚𝐛𝐲🥳...")
    await userbot.start()
    await DAXX.start()
    try:
    ...
    LOGGER("INSANEMUSIC").info(
        "╔═════ஜ۩۞۩ஜ════╗\n  ☠︎︎𝗠𝗔𝗗𝗘 𝗕𝗬 𝗠𝗥 KANEKI ☠︎︎\n╚═════ஜ۩۞۩ஜ════╝"
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("INSANEMUSIC").info("𝗦𝗧𝗢𝗣 𝗜𝗡𝗦𝗔𝗡𝗘 𝗠𝗨𝗦𝗜𝗖🎻 𝗕𝗢𝗧..")
